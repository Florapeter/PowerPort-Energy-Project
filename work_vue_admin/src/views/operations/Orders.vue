        ...
        <el-table :data="dataList" v-loading="loading" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="订单状态" prop="status">
                <template #default="scope">
                    ...
                    <el-tag type="warning" v-else-if="scope.row.status == 4">进行中</el-tag>
                </template>
            </el-table-column>
...
const {setRowData} = orderStore
const {addTab,setCurrentTab} = tabsStore
const handleDetail = (row: SelectionListType) => {
...
const exportToExcel=()=>{
    const ws = XLSX.utils.json_to_sheet(selectionList.value);//把数据转成工作表格
