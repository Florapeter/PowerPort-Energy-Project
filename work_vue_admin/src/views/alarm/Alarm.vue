<template>
    ...
    >
        ...
        <el-descriptions :border="true" :column="4" direction="vertical" class="mt">
            <el-descriptions-item v-for="(val, key) in item" :label="getLabel(key)">
                ...
                >
                    {{ val == 1 ? '紧急' : val == 2 ? '严重' : '一般' }}
                </el-tag>
                <el-text v-else-if="key == 'status'" :type="val == 1 ?'warning':(val == 2 ? 'success':'danger')">
                    {{ val == 1 ? '待指派' : val == 2 ? '处理中' : '处理异常' }}
                </el-text>
                <span v-else>{{ val }}</span>
            </el-descriptions-item>
            <el-descriptions-item label="操作">
                <el-button  @click="openDrawer(item.equNo)" :type="item.status == 2 ? 'warning' : 'primary'">{{
    ...
    <el-drawer v-model="drawer" title="报警任务指派">
        ...
        >
            ...
            <template #step-2>
                <el-form :model="formData.shenpi" ref="form2" :rules="shenpiRules">
                    <el-form-item label="审批部门" prop="a">
                        <el-select v-model="formData.shenpi.a" placeholder="请选择审批部门">
            ...
            <template #step-3>
                <el-form ref="form3" :model="formData.info" :rules="infoRules">
                    <el-form-item label="负责人姓名" prop="person">
                        <el-input v-model="formData.info.person" placeholder=""></el-input>
                    </el-form-item>
                    <el-form-item label="负责人电话" prop="tel">
                        <el-input v-model="formData.info.tel" placeholder=""></el-input>
                    </el-form-item>
                </el-form>
            </template>
        </StepForm>
        <el-result icon="warning" :title="currentEquipNo" sub-title="该任务已催促2次，请抓紧处理">
            <template #extra>
...
const currentEquipNo = ref<string>('')
...
const openDrawer = (equipNo: string) => {
    drawer.value = true
    currentEquipNo.value = equipNo
}
