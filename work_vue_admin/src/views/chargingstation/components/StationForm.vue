<template>
    <el-dialog :model-value="dialogVisible" :title="title" @close="handleCancel" destroy-on-close>
        <el-form label-width="120px" :rules="rules" :model="ruleForm" ref="formRef">
            <el-row>
                <el-col :span="12">
...
const rules = reactive<FormRules<RowType>>({
    ...
    city: [{ required: true, message: '站点城市不能为空', trigger: 'blur' }],
    person: [{ required: true, message: '站点负责人不能为空', trigger: 'blur' }],
    tel: [{ required: true, message: '站点电话不能为空', trigger: 'blur' }],
...
const { rowDate } = storeToRefs(stationStore)
const title = ref<string>('')

watch(
    () => props.dialogVisible,
    () => {
        if (rowDate.value.name) {
            title.value = '编辑充电站信息'
            disabled.value = true
        } else {
            title.value = '新增充电站信息'
